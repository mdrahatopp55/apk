...
✏️ /rename old new`
  ...
  if (msg.text?.startsWith("/delete ")) {
    ...
    const r = await fetch(`${GH_API}/repos/${GH_REPO}/contents/${file}`, { headers: ghHeaders });
    if (r.status !== 200) {
      await send("❌ File not found");
      return res.end();
    }
    const j = await r.json();
    ...
    await fetch(`${GH_API}/repos/${GH_REPO}/contents/${file}`, {
      method: "DELETE",
      headers: ghHeaders,
      body: JSON.stringify({ message: "delete", sha: j.sha, branch: BRANCH })
    });
  ...
  if (msg.text?.startsWith("/rename ")) {
    ...
    const r = await fetch(`${GH_API}/repos/${GH_REPO}/contents/${oldN}`, { headers: ghHeaders });
    if (r.status !== 200) {
      await send("❌ File not found");
      return res.end();
    }
    ...
    await fetch(`${GH_API}/repos/${GH_REPO}/contents/${newN}`, {
      method: "PUT",
      headers: ghHeaders,
      body: JSON.stringify({ message: "rename", content: j.content, branch: BRANCH })
    });

    await fetch(`${GH_API}/repos/${GH_REPO}/contents/${oldN}`, {
      method: "DELETE",
      headers: ghHeaders,
      body: JSON.stringify({ message: "remove old", sha: j.sha, branch: BRANCH })
    });
  ...
  if (!file) {
    await send("📤 Send a file");
    return res.end();
  }

  const fInfo = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/getFile?file_id=${file.file_id}`).then(r => r.json());
  ...
  await fetch(`${GH_API}/repos/${GH_REPO}/contents/${name}`, {
    method: "PUT",
    headers: ghHeaders,
    body: JSON.stringify({ message: "upload", content: base64, branch: BRANCH })
  });
